...
if TYPE_CHECKING:
    ...
    from .vlnm_commands_cfg import  VLNMVelocityCommandCfg

# pdj: import language related packages
from transformers import AutoTokenizer, AutoModel 


class VLNMVelocityCommand(CommandTerm):
    ...
    cfg: VLNMVelocityCommandCfg
    """The configuration of the command generator."""

    def __init__(self, cfg: VLNMVelocityCommandCfg, env: ManagerBasedEnv):
        ...
        # -- command: encoded language
        # 1. load language model. device_map=self.device;
        ...
        # 4. init language_vel_command and apply the pooler to it, shape is [num_envs, 768]
        self.language_vel_command = torch.zeros(self.num_envs, self.env_id_context.shape[1], device=self.device) 
    ...
    def _resample_command(self, env_ids: Sequence[int]):
        ...
        # pdj: update language commands
        self._encode_language_commands(self.vel_command_b, env_ids)
    ...
    def _update_command(self):
        ...
        # pdj: update language commands
        self._encode_language_commands(self.vel_command_b, standing_env_ids)

    def _encode_language_commands(self, vel_command_b, env_ids: Sequence[int]):
        '''pdj: encode the language commands'''
        # 1. tokenize the digital command expression
        self.env_id_tokens = self.tokenizer(
                "I am running at longitudinal speed: {} m/s, lateral speed: {} m/s, spinning speed: {} m/s.".format(vel_command_b[0, 0], vel_command_b[0, 1], vel_command_b[0, 2]), 
